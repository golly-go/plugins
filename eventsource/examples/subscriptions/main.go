...
func main() {
	...
	engine.Subscribe("orders", "OrderUpdated", func(ctx *golly.Context, evt eventsource.Event) {
		log.Printf("Handling order updated event: %s", evt.Type)
	})

	// Subscribe to aggregate type
	engine.SubscribeAggregate("orders", "Customer", func(ctx *golly.Context, evt eventsource.Event) {
